...
export default function AdminDashboard() {
  ...
  const loadAdminData = async () => {
    try {
      ...
      // Charger l'activité récente SANS jointure
      ...
      if (recentError) {
        console.warn('Erreur chargement activité récente:', recentError);
      } else if (recentTransactions && recentTransactions.length > 0) {
        // Extraire les user IDs (avec filtrage des null)
        const activityUserIds = [...new Set(recentTransactions.map(t => t.user_id).filter(Boolean))];

        // Charger les profiles pour l'activité récente
        let profilesForActivity: any[] = [];
        if (activityUserIds.length > 0) {
          ...
          if (activityProfilesError) {
            console.warn('Erreur profiles activité:', activityProfilesError);
          } else {
            profilesForActivity = activityProfiles || [];
          }
        }

        // Créer un map pour lookup rapide
        const activityProfilesMap = Object.fromEntries(
          profilesForActivity.map(profile => [profile.id, profile.full_name])
        );

        // Construire l'activité récente avec jointures manuelles
        const activities: RecentActivity[] = recentTransactions.map(t => ({
          id: t.id,
          type: 'transaction',
          description: `Nouveau don de ${t.amount}€`,
          timestamp: t.created_at,
          user_name: activityProfilesMap[t.user_id] || 'Utilisateur inconnu',
